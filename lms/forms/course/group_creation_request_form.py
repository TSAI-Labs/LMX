...
def get_students(user_id, course_id): 
    ...
    STUDENT_EMAILS = [
   ( StudentCourse.user.email+","+StudentCourse.user.username,StudentCourse.user.username,) for StudentCourse in StudentCourse.objects.filter(courses_id = course_id, group = None).exclude(user_id =user_id)
