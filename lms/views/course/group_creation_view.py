                ...
                message = render_to_string('lms/course/group_creation_request_email.html',
                                           {
                                               ...
                                               'uid': urlsafe_base64_encode(
                                                   force_bytes(str(user.pk) + "+" + group_name)),
            ...
            if user is not None and group_name is not None and account_activation_token.check_token(user, token):
                ...
                if (request.user == user):
                    ...
                    messages.error(request,
                                   f"You are not the one whom the request is sent to or you may be already existed in some group"
                                   )
...
class ViewGroupsView(LoginRequiredMixin, ListView):
    """
